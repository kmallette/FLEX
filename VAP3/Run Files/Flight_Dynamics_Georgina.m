...
filename = 'inputs/Flight_Dynamics_George.vap';
% For IP reasons the original airfoil used for this code has been replaced
% with the NACA2412.
...
% Parametric sweep data to load (assumes constant properties across the span)
INPU.matEIx_param = INPU.vecEIxCOEFF(1,3);
INPU.matGJt_param = INPU.vecGJtCOEFF(1,3);
INPU.vecEA_param = INPU.vecEACOEFF(1,3);
INPU.vecCG_param = INPU.vecCGCOEFF(1,3);
...
FLAG.TRIM = 0;
FLAG.VISCOUS = FLAG.VISCOUS; % Turn viscous calcs on (1) or off (0)
...
FLAG.STIFFWING = FLAG.STIFFWING;
FLAG.GIF = FLAG.GIF;
%% Perform full flight-dynamic simulation on trimmed/deformed aircraft
SURF.matBEAMACC = [];
COND.valGUSTAMP = COND.valGUSTAMP;
COND.valGUSTSTART = COND.valGUSTSTART;
...
COND.valMAXTIME = ceil((2*COND.valGUSTL + SURF.valTBOOM)/COND.vecVEHVINF/COND.valDELTIME + COND.valGUSTSTART); % Max time is 5 gust lengths
COND.valSTIFFSTEPS = stiff_step; % Don't compute structure deflections for 15 timesteps
COND.valSTARTFORCES = 1; % Start force calcs at timestep 1
FLAG.FLIGHTDYN = FLAG.FLIGHTDYNAMICS; % Flight dynamics model on (1) or off (0) FLAG.FLIGHTDYN
FLAG.STATICAERO = 0;
FLAG.STEADY = FLAG.STEADY; % Steady (1) or unsteady (2) aerodynamics
FLAG.RELAX = FLAG.RELAX; % Don't set this to 1 unless you want to have a bad time
FLAG.GUSTMODE = GustModeTemp; % Sine (1), 1-cosine (2), sharp-edge (3), von Karman (4) [MUST LOAD GUST PROFILE IF YOU DON'T WANT IT TO CHANGE EVERY RUN] or no gust (0)
FLAG.SAVETIMESTEP = save_time;
